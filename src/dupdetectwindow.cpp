...
static void relabel_hash_parent(MyTreeItem* item)
{
    if (!item || !item->label()) {
    ...
    // hash length + space for number of files and message
    new_label.reserve(64 + 20);
    new_label.append("Hash: ");
    new_label.append(item->get_hash());
    new_label.append(" (");
    new_label.append(std::to_string(item->children()));
    new_label.append(" files)");
    item->label(new_label.c_str());
}
...
std::shared_ptr<DupDetectWindow> DupDetectWindow::construct_window()
{
    ...
    {
        w->My_resultsTree = new MyTree(15, 180, 740, 340);
...
static MyTreeItem* try_get_selected_item(MyTree* tree)
{
    auto* item = static_cast<MyTreeItem*>(tree->first_selected_item());
    if (item == nullptr) {
...
{
    auto* item =
        static_cast<MyTreeItem*>(My_resultsTree->first_selected_item());
    ...
    if (item->is_directory() || item == My_resultsTree->root()) {
...
{
    MyTreeItem* item = try_get_selected_item(My_resultsTree);
    if (item == nullptr) {
        return;
    }
...
void DupDetectWindow::delete_single_item(MyTreeItem* item)
{
    ...
    if (parent == My_resultsTree->root()) {
        ::output(
            "Cannot delete single item when parent is root. Maybe you meant to "
            "call delete_hash_parent_item. Application will "
            "terminate!");
        die();
    }

    auto p = static_cast<MyTreeItem*>(parent);

    if (item->is_survivor() && parent->children() > 1) {
        ...
        // removed//
        choose_survivor_after_delete(p, item);
    }
    ...
    if (parent->children() > 0) {
        relabel_hash_parent(p);
    } else {
        My_resultsTree->remove(p);
    }
}

void DupDetectWindow::delete_hash_parent_item(MyTreeItem* item, int child_count)
{
    // find the child with the user data indicating it is
    // the newest and should be saved
    for (auto i = child_count - 1; i >= 0; --i) {
        if (auto* child = static_cast<MyTreeItem*>(item->child(i));
            child && !child->is_survivor()) {
    ...
    MyTreeItem* old_survivor)
{
    ...
    for (int i = parent->children() - 1; i >= 0; --i) {
        auto* child = static_cast<MyTreeItem*>(parent->child(i));
    ...
    auto* new_child_survivor =
        static_cast<MyTreeItem*>(parent->find_child_item(new_survivor.data()));
    ...
    new_child_survivor->set_file_data(new_child_survivor->get_path(),
                                      new_child_survivor->get_hash(),
                                      FileData::Type::FILE, true);
    new_child_survivor->labelfgcolor(FL_BLUE);
    old_survivor->set_file_data(old_survivor->get_path(),
                                old_survivor->get_hash(), FileData::Type::FILE,
                                false);
    return new_survivor;
}
    ...
    std::shared_ptr<DuplicateFilesCollection> duplicateFiles)
{
    ...
    for (const auto& [hash, hash_entry] : *duplicateFiles) {
        ...
        if (hash_entry.get_file_count() > 1) {
            ...
            parent->set_file_data(text, hash, FileData::Type::DIRECTORY, false);
            auto survivor = choose_survivor(files);
            for (const auto& file : files) {
                auto* item = My_resultsTree->add(parent, file.c_str());
                // temporarily mark all as non-survivors
                item->set_file_data(file, hash, FileData::Type::FILE, false);
                item->labelfgcolor(FL_GRAY);
            }
            auto* sur = static_cast<MyTreeItem*>(
                parent->find_child_item(survivor.data()));
            sur->set_file_data(sur->get_path(), sur->get_hash(),
                               FileData::Type::FILE, true);
